...
sudo bash install_optimized.sh
```
...
```bash
# 备份数据库
docker exec mysql5.7 mysqldump -u root -p<password> --all-databases > backup.sql

# 恢复数据库
docker exec -i mysql5.7 mysql -u root -p<password> < backup.sql
```
